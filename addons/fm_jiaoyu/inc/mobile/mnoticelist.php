 ...
 */
        ...
        if(!empty($userid['id'])){
		    ...
		    // $leave = pdo_fetchall("SELECT * FROM " . tablename($this->table_notice) . " where :schoolid = schoolid And :weid = weid And :tid = tid And :type = type ORDER BY createtime DESC", array(
				 ...
				 // ));
            //取本登录老师的群发消息（校长不取）
			$leave = pdo_fetchall("SELECT * FROM " . tablename($this->table_notice) . " where :schoolid = schoolid And :weid = weid And :type = type ORDER BY createtime DESC", array(
		         ':weid' => $weid,
				 ':schoolid' => $schoolid,
				 ':type' => 2
				 ));
		 ...
		 include $this->template('mnoticelist');
          }else{
