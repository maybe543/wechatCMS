...
class TplConfigService extends CommonService
{
    ...
    {
        $this->table_name = 'lonaking_supertask_tpl_template_config';
        $this->columns = 'id,uniacid,get_notice,check_status_access_notice,check_status_refuse_notice,send_notice,invite_score_notice';
        $this->giftOrderService = new GiftOrderService();
    ...
    public function initTplConfig(){
        ...
        $tplConfig = array(
            ...
            'send_notice' => '',//发货
            'invite_score_notice' => '',//邀请关注积分变更
        );
//        $tplConfig = array(
//            'uniacid' => $uniacid,
//            'get_notice' => $this->fetchTemplateIdByShort('OPENTM207444890'),//礼品兑换成功提醒
//            'check_status_access_notice' => $this->fetchTemplateIdByShort('OPENTM207508435'),//审核成功
//            'check_status_refuse_notice' => $this->fetchTemplateIdByShort('OPENTM207508435'),//审核失败
//            'send_notice' => $this->fetchTemplateIdByShort('OPENTM207464068'),//发货
//            'invite_score_notice' => $this->fetchTemplateIdByShort('OPENTM207421487'),//邀请关注积分变更
//        );
     ...
     */
    public function sendGetGiftSuccessTplNotice($orderOrId,$module_config){
        $order = null;
        ...
        $user = $this->userService->selectUserByOpendiOrFanid($order['openid'],$module_config);
        $this->log($user);
    ...
    public function sendGiftOrderCheckStatusAccessNotice($orderOrId){
        ...
        $this->log('订单信息为:'.json_encode($order));
        global $_W;
        $account = $this->createWexinAccount();
        $config = $this->checkConfigByUniacid($_W['uniacid']);
    ...
    public function sendGiftOrderCheckStatusRefuseNotice($orderOrId){
        ...
        $postData = array(
            ...
            'remark' => array(
                'value' => '您的兑换由于某些原因我们无法通过，我们已将礼品积分退回您的账户，为您带来的不便非常抱歉，感谢你对我们的支持',
                'color' => '#FF683F'
            )
        );
        $sendArray = array(
    ...
    public function sendGiftSendUpTplNotice($orderOrId){
        ...
        $postData = array(
            ...
            'remark' => array(
                'value' => '您的礼品已经由快递发出，由于系统原因，快递公司名称未能显示，您可以在百度查询运单号关注物流动态',
                'color' => '#FF683F'
            )
        );
        $sendArray = array(
    ...
    /**
     * 获取微信account
     * @return AlipayAccount|null|WeiXinAccount|YiXinAccount
     */
    private function createWexinAccount(){
        global $_W;
        load()->classs('weixin.account');
