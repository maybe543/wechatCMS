...
class q_3354988381_xuyuanModuleSite extends WeModuleSite
{
    ...
    public function doWebManage()
    {
        ...
        $params[':module'] = 'q_3354988381_xuyuan';
        ...
        if (!empty($list)) {
            foreach ($list as &$item) {
                ...
                $scratch           = pdo_fetch("SELECT * FROM " . tablename('dream_reply') . " WHERE rid = :rid ", array(
                    ':rid' => $item['id']
                ));
    ...
    public function doWebWishlist()
    {
        ...
        $reply    = pdo_fetch("SELECT * FROM " . tablename($this->tablename) . " WHERE rid = :rid", array(
            ':rid' => $rid
        ));
    ...
    public function doWebSetshow()
    {
        ...
        $temp = pdo_update('dream_reply', array(
            'isshow' => $isshow
        ), array(
            'rid' => $rid
        ));
    ...
    public function doWebSetstatus()
    {
        ...
        $temp = pdo_update('dream_wish', $p, array(
            'id' => $id,
            'weid' => $_W['uniacid']
        ));
        if ($temp == false) {
            ...
            message('状态设置成功！', $this->createWebUrl('wishlist', array(
                'rid' => $_GPC['rid']
            )), 'success');
        }
    }
    public function doMobileIndex()
    {
        ...
        $reply = pdo_fetch("SElECT * FROM" . tablename($this->tablename) . "WHERE rid=:rid LIMIT 1", array(
            ':rid' => $id
        ));
        if ($reply == false) {
    ...
    public function doMobileWish()
    {
        ...
        $flag      = pdo_update("dream_reply", array(
            'dreamnum' => $reply['dreamnum'] + 1
        ), array(
            'id' => $reply['id']
        ));
    }
