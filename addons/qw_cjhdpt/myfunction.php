...
function j_tempeleSendMessage($id=0,$funid=0,$openid="",$data=array(),$remark="",$url="",$appids="",$appsecrets="") {
	...
	$actItem = pdo_fetch("SELECT * FROM ".tablename('qw_cjhdpt_reply')." where id='".$funid."' ");
	$userInfo = pdo_fetch("SELECT * FROM ".tablename('qw_cjhdpt_winner')." where (from_user='".$openid."' or openid='".$openid."') and aid='".$funid."'  limit 1 ");
...
function jetsum_sendMessage($openid="",$content="",$aid="",$remark=""){
	...
	$actItem=pdo_fetch("SELECT * FROM ".tablename('qw_cjhdpt_reply')." WHERE rid = '".$aid."'");
	$userInfo=pdo_fetch("SELECT * FROM ".tablename('qw_cjhdpt_winner')." WHERE aid = '".$actItem['id']."' and from_user='".$openid."' ");
	...
	pdo_insert("qw_cjhdpt_msgrecord",$data);
	return true;
}
