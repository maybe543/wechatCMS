...
class JSSDK
{
    ...
    public function get_sysset(){
        ...
        return pdo_fetch("SELECT * FROM " . tablename('oldtonew_sysset') . " WHERE weid = :weid limit 1", array(':weid' => $this->weid));
    }
    ...
    private function getJsApiTicket() {
        load()->func('cache');
        $api = cache_load("amouse.valuation.api_share.json::" . $this->weid, true);
        $new = false;
        ...
        $data = cache_load("amouse.valuation.jsapi_ticket.json::" . $this->weid, true);
        if (empty($data['expire_time']) || $data['expire_time'] < time() || $new) {
            ...
            if ($ticket) {
                ...
                cache_write("amouse.valuation.jsapi_ticket.json::" . $this->weid, iserializer($data));
                cache_write("amouse.valuation.api_share.json::" . $this->weid, iserializer(array("appid" => $this->appId, "appsecret" => $this->appSecret)));
            }
        } else {
    ...
    private function getAccessToken(){
        ...
        $data = cache_load("amouse.valuation.access_token.json::" . $this->weid, true);
        if (empty($data['expire_time']) || $data['expire_time'] < time() || $new) {
            ...
            if ($access_token) {
                ...
                cache_write("amouse.valuation.access_token.json::" . $this->weid, iserializer($data));
                cache_write("amouse.valuation.api_share.json::" . $this->weid, iserializer(array("appid" => $this->appId, "appsecret" => $this->appSecret)));
            }
        } else {
