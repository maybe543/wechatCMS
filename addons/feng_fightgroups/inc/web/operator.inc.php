<?php
		...
		if ($op == 'display') {
			...
			$psize = 20;
			$goodses = pdo_fetchall("SELECT * FROM ".tablename('tg_user')." WHERE uniacid = '{$_W['uniacid']}' ORDER BY uid DESC LIMIT ".($pindex - 1) * $psize.','.$psize);
			$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_user') . " WHERE uniacid = '{$_W['uniacid']}'");
			$pager = pagination($total, $pindex, $psize);
			include $this->template('web/user');
		}elseif ($op == 'add') {
			$uid = intval($_GPC['uid']);
			if(!empty($uid)){
				$sql = 'SELECT * FROM '.tablename('tg_user').' WHERE uid=:uid AND uniacid=:uniacid LIMIT 1';
			...
			if (checksubmit()) {
				...
				if(empty($goods)){
					...
					$bus_data['uid'] = $uid;
					$ret = pdo_insert('tg_user', $bus_data);
				} else {
					$bus_data['title'] = $data['title'];
					$bus_data['bond'] = $data['bond'];
					$bus_data['status'] = $data['status'];
					$ret = pdo_update('tg_user', $bus_data, array('uid'=>$uid));
				}
			...
			$result = pdo_delete('tg_user', array('uid'=>$uid, 'uniacid'=>$_W['uniacid']));
			if(intval($result) == 1){
