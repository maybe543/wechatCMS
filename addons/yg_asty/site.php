...
class Yg_astyModuleSite extends WeModuleSite
{
    public $table_reply = 'yg_asty_reply';
    public $table_info = 'yg_asty_info';
    public $table_recomd = 'yg_asty_recomd';
    public $table_user = 'yg_asty_user';
    ...
    public function doMobileupdatecount()
    {
        ...
        if ($member['forward'] >= $reply['forward']) {
            echo 1;
        } else {
            $addnum  = $member['count'] + $reply['addnum'];
            $forward = $member['forward'] + 1;
    ...
    public function doMobilecheckcount()
    {
        global $_W, $_GPC;
        $uniacid  = $_W['uniacid'];
        $userinfo = $this->userinfo;
        ...
        if ($member['comparetime'] == $today) {
            $recomdnum = pdo_fetchcolumn("SELECT count(*) FROM " . tablename($this->table_recomd) . " where uniacid={$uniacid} and openid='{$openid}' and rid ={$reply['rid']}");
            if ($recomdnum < $member['count']) {
                echo 1;
            } else {
                echo 2;
            }
    ...
    public function doMobilemored()
    {
        ...
        foreach ($userall as $key => $value) {
            $i++;
            $html .= "<li><div class='NO'>{$i}</div><div class='Head'><img src=" . $value['headimgurl'] . " width='100%'></div><div class='text'><div class='his'><h1>" . $value['nickname'] . "</h1><h2>" . $value['tel'] . "</h2></div><div class='gift'> <p>" . $value['asnum'] . "<span>个</span></p></div></div></li>";
        }
