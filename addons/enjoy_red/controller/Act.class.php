...
class Act {
	public function getact(){
		global $_W;
		//取活动信息
		$act=pdo_fetch("select * from ".tablename('enjoy_red_reply')." where uniacid=".$_W['uniacid']."");
		return $act;
	}
    ...
    public function confirm($id) {
        ...
        $ret = pdo_update('mbrp_records', $rec, $filters);
        if(!empty($ret)) {
            $sql = 'SELECT * FROM ' . tablename('mbrp_records') . " WHERE `id`=" . $id;
            $record = pdo_fetch($sql);
            if(!empty($record)) {
                require_once MB_ROOT . '/source/Fans.class.php';
                $f = new Fans();
                $fans = $f->getOne($record['uid']);
                
                $r = array_elements(array('id', 'uid', 'activity', 'gift', 'fee', 'log', 'status', 'created', 'completed'), $record);
                $r = array_merge($r, array_elements(array('openid', 'proxy', 'nickname', 'gender', 'state', 'city', 'avatar', 'uid'), $fans));
                $this->saveRecent($record['activity'], $r);
            }
        }
        return $ret;
    }
    ...
    private function saveRecent($actid, $record) {
        $key = "mbrp:activity:{$actid}:recents";
    ...
    public function getRecents($actid, $limit = 5) {
        $recents = cache_read("mbrp:activity:{$actid}:recents");
        if(empty($recents)) {
            $recents = array();
        }
        ...
        if(empty($ret)) {
            ...
            $ds = $this->getRecords($filters, 1, 10);
            if(!empty($ds)) {
                foreach($ds as $row) {
