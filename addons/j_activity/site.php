...
class J_activityModuleSite extends WeModuleSite {
	...
	public function doMobileAjax() {
		...
		pdo_update("j_activity_winner",array('attend'=>1,'endtime'=>TIMESTAMP),array('id'=>$item['id']));
		...
		if($cfg['tempmsg_on'] && $cfg['tempmsg_join_sign']){
			$url=$_W['siteroot']."app/index.php?i={$_W['uniacid']}&c=entry&do=view&m=j_activity&id=".$rid['id']."&wxref=mp.weixin.qq.com#wechat_redirect";
	...
	public function doMobileAjaxuser() {
		...
		$rid=pdo_fetch("SELECT * FROM ".tablename('j_activity_reply')." WHERE id = '".$id."' ");
		$item=pdo_fetch("SELECT * FROM ".tablename('j_activity_winner')." WHERE aid = '".$id."' and from_user='".$openid."'");
	...
	public function doMobileAjaxgetinfo() {
		...
		$item=pdo_fetch("SELECT * FROM ".tablename('j_activity_winner')." WHERE id=:id",array(":id"=>intval($_GPC['id'])));
		if(empty($item))die(json_encode(array('success'=>false,'msg'=>"报名人员不存在！")));
		$reply=pdo_fetch("SELECT * FROM ".tablename('j_activity_reply')." WHERE id=:id",array(":id"=>intval($item['aid'])));
	...
	public function doMobileAjaxget(){
		...
		if($operation=='getlist'){
			...
			$list = pdo_fetchall("SELECT * FROM ".tablename('j_activity_reply')." WHERE weid = '{$_W['uniacid']}' order by ".$orderstr." LIMIT ".$pindex*$psize.",".$psize);
			$total = pdo_fetchcolumn("SELECT * FROM ".tablename('j_activity_reply')." WHERE weid = '{$_W['uniacid']}' ");
			$tempary=array();
			foreach($list as $row){
				$temp=array();
				...
				$temp['joinnum']=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('j_activity_winner')." WHERE aid ='".$row['id']."'");
				$temp['ccate']=$row['ccate'];
				$temp['address']=$row['address'];
				$temp['label']=$row['label'];
				$temp['status']=TIMESTAMP>$row['endtime'] ? 1:0;
				if($row['ccate']>0){
					$p=pdo_fetch("SELECT * FROM ".tablename('j_activity_category')." WHERE id = '".$row['ccate']."' ");
					if($p){
						$temp['label']=$p['name'];
		...
		if($operation=='submitreg'){
			...
			if ($_W['ispost']){
				if($_GPC['from_user']){
					$isjoin=pdo_fetch("SELECT id FROM ".tablename('j_activity_winner')." WHERE from_user='".$_GPC['from_user']."' and aid='".$_GPC['id']."'");
					//if($isjoin)die(json_encode(array('success'=>false,'msg'=>"不能重复报名哦")));
				}
				$item=pdo_fetch("SELECT * FROM ".tablename('j_activity_reply')." WHERE id = :id",array(':id'=>$_GPC['id'],));
				$insert=array(
				...
				if(empty($isjoin)){
					...
					pdo_update("j_activity_winner", $insert,array("id"=>$isjoin['id']));
				}
				//---报名成功后---//
				...
				if($cfg['tempmsg_on'] && $cfg['tempmsg_join_call']){
					$url=$_W['siteroot']."app/index.php?i={$_W['uniacid']}&c=entry&do=view&m=j_activity&id=".$_GPC['id']."&wxref=mp.weixin.qq.com#wechat_redirect";
	...
	public function doMobileAppend() {
		...
		if($operation=='ok'){
			$list=pdo_fetchall("SELECT * FROM ".tablename('j_activity_winner')." WHERE aid = :aid order by attend desc,status desc,id desc",array(':aid'=>$item['id']));
	...
	public function doMobileView(){
		...
		if($today>$item['endtime']){
			...
			if($item['status']==0){
				if($today>=$item['joinstarttime'] && $today<=$item['joinendtime']){
					pdo_update('j_activity_reply',array('status'=>1),array('id'=>$id));
					$item=pdo_fetch("SELECT * FROM ".tablename('j_activity_reply')." WHERE id = :id",array(':id'=>$id));
				}
			}
		}
		$parama=json_decode($item["parama"],true);
		if($from_user){
			$member=pdo_fetch("SELECT * FROM ".tablename('j_activity_winner')." WHERE aid = '".$item['id']."' and from_user='".$from_user."' ");
		}
		//---访问记录---//
		pdo_update('j_activity_reply',array('visitied'=>$item['visitied']+1),array('id'=>$id));
		//---访问记录---//
		if($_W['openid'] || $_W['clientip']){
			$visiter=pdo_fetch("SELECT * FROM ".tablename('j_activity_record')." WHERE aid='".$id."' and (from_user='".$_W['openid']."' or ip='".$_W['clientip']."') order by id desc limit 1");
			if(empty($visiter) || TIMESTAMP-$visiter['createtime']>3600){
				...
				pdo_insert('j_activity_record',$visidata);
			}
		}
		//=====显示报名情况，最新报名人员=====//
		$newjoinerList=pdo_fetchall("SELECT * FROM ".tablename('j_activity_winner')." WHERE aid = '".$item['id']."' order by id desc ");
		$joincount=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('j_activity_winner')." WHERE aid = '".$item['id']."' ");
		//========初始化参数=========//
		$parmaTemp="";
		...
		if($item['charge']>0 && $member && $cfg['is_pay'] && $member['paystatus']==0 && $member['paytime']==0  && !$member['transid']){
			...
			if(!$tid){
				...
				pdo_update("j_activity_winner",array('ordersn'=>$tid),array('id'=>$member['id']));
			}
			$params['tid'] = $tid;
			...
			if(!empty($log) && $log['status'] == '1'){
				pdo_update("j_activity_winner",array('paystatus'=>1,'status'=>2,'paytime'=>TIMESTAMP,'transid'=>$log['plid']),array('ordersn'=>$tid));
	...
	public function payResult($params) {
        ...
        pdo_update('j_activity_winner', $data, array('ordersn' => $params['tid']));
		$user=pdo_fetch("SELECT * FROM ".tablename('j_activity_winner')." WHERE ordersn = :ordersn",array(":ordersn"=>$params['tid']));
		$openid=$user['openid'];
		$item=pdo_fetch("SELECT * FROM ".tablename('j_activity_reply')." WHERE id = :id",array(":id"=>$user['aid']));
		$cfg = $this->module['config'];
        ...
        $url=$_W['siteroot']."app/index.php?i=".$_W['uniacid']."&c=entry&id=".$item['id']."&do=view&m=j_activity";
	...
	public function doMobileAutooath() {
		...
		if(!isset($_GPC['code'])){
			$url=$_W['siteroot']."app/index.php?i=".$_W['uniacid']."&c=entry&id=".$_GPC['id']."&from_user=".$_GPC['from_user']."&do=autooath&m=j_activity";
	...
	public function doMobileError() {
		...
		global $_GPC, $_W;
		$id=$_GPC['id'];
		$rid = pdo_fetchcolumn("SELECT rid FROM ".tablename('j_activity_reply')." WHERE id = :id LIMIT 1", array(':id' => $id));
	...
	public function doWebJoiner() {
		...
		$list = pdo_fetchall("SELECT * FROM ".tablename('j_activity_winner')." WHERE aid = '".$item['id']."' $where order by $order");
		...
		$count2=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('j_activity_winner')." WHERE aid = '".$item['id']."' and status=2");
		...
		if($operation=='in'){
			if(!empty($uid)){
				...
				if($cfg['tempmsg_on'] && $cfg['tempmsg_join_ok'] && $item['status']<2){
					$url=$_W['siteroot']."app/index.php?i={$_W['uniacid']}&c=entry&do=view&m=j_activity&id=".$item['id']."&wxref=mp.weixin.qq.com#wechat_redirect";
					if($cfg['user_oauth'])$openid=pdo_fetchcolumn("SELECT openid FROM ".tablename('j_activity_winner')." WHERE id = '".$uid."' ");
			...
			$uid=intval($_GPC['uid']);
			$profile=pdo_fetch("SELECT * FROM ".tablename('j_activity_winner')." WHERE id = '".$uid."' ");
			$list=pdo_fetchall("SELECT * FROM ".tablename('j_activity_msgrecord')." WHERE aid = '".$item['id']."' and (from_user='".$profile['from_user']."' or from_user='".$profile['openid']."') order by id desc");
		}
	...
	public function doWebManage() {
		...
		if ($operation == 'display') {
			$list = pdo_fetchall("SELECT * FROM ".tablename('j_activity_reply')." WHERE weid = '{$_W['uniacid']}' order by id desc");
		} elseif ($operation == 'post') {
			
			if(!empty($id)) {
				$reply = pdo_fetch("SELECT * FROM ".tablename('j_activity_reply')." WHERE id = '$id'");
			}
			$grouplist= pdo_fetchall("SELECT * FROM ".tablename("mc_groups")." WHERE uniacid = '".$_W['uniacid']."' ORDER BY `orderlist` asc");
			if (checksubmit('submit')) {
				...
				$insert['parama']=urldecode(json_encode($parama));
				if (!empty($id)) {
					pdo_update('j_activity_reply', $insert, array('id' => $id));
				}
	...
	public function doWebExport() {
		global $_GPC, $_W;
		$operation = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
		require_once '../addons/j_activity/PHPZip.php';
		require_once '../framework/library/phpexcel/PHPExcel.php';
		$rid=intval($_GPC['rid']);
		load()->func('file');
		$item = pdo_fetch("SELECT * FROM ".tablename('j_activity_reply')." WHERE rid =:id ",array(":id"=>$rid));
		$list = pdo_fetchall("SELECT * FROM ".tablename('j_activity_winner')." WHERE aid =:id order by id desc",array(":id"=>$item['id']));
		if($operation=="excel"){
			$parama=json_decode($item['parama'],true);
			...
			foreach($parama as $index=>$p){
				if($p==1)array_push($tableheader,$index);
			}
			$html = "\xEF\xBB\xBF";
			...
			foreach($list as $row){
				...
				$html .= $status . "\t ,";
				$attend=$row['attend']? '已签到':'';
				$html .= $attend . "\t ,";
				$html .= $row['reloadmsg'] . "\t ,";
				$html .= $row['remark'] . "\t ,";
				$para=json_decode($row['parama'],true);
				foreach($parama as $index=>$p){
					if($p==1)$html .= $para[$index] . "\t ,";
				}
				$html .="\n";
			}
			header("Content-type:text/csv");
			header("Content-Disposition:attachment; filename=用户数据-$rid.csv");
			echo $html;
			exit();
			
		}
		if($operation=="allinfo"){
			if(is_dir("../addons/j_activity/temp/".$_W['uniacid']."/".$rid))rmdirs("../addons/j_activity/temp/".$_W['uniacid']."/".$rid);
			if(!is_dir("../addons/j_activity/temp/"))@mkdirs("../addons/j_activity/temp/");
			$outputFlorder="../addons/j_activity/temp/".$_W['uniacid']."/";
			...
			$outputFlorder="../addons/j_activity/temp/".$_W['uniacid']."/".$rid;
			if(!is_dir($outputFlorder))@mkdirs($outputFlorder);
	...
	public function doWebScreen() {
		global $_GPC, $_W;
		$id = intval($_GPC['id']);
		$reply = pdo_fetch("SELECT * FROM ".tablename('j_activity_reply')." WHERE rid = '$id'");
		$time=pdo_fetchcolumn("SELECT max(times) FROM ".tablename('j_activity_lucky')." WHERE aid = '".$reply['id']."' limit 1");
		$times=intval($time)+1;
		$operation = !empty($_GPC['op']) ? $_GPC['op'] : 'display';
		if($operation=="restart"){
			if($id){
				pdo_delete("j_activity_lucky",array("aid"=>$reply['id']));
			}
			message('系统初始化完成！', $this->createWebUrl('screen', array('op' => 'display','id' => $id)), 'success');
		}
		$gamenumber = pdo_fetchcolumn("SELECT count(*) FROM ".tablename('j_activity_lucky')." WHERE aid ='".$reply['id']."'");
	...
	public function doWebLucky() {
		...
		if($_GPC['wid']){
			...
			$wid=intval($_GPC['wid']);
			if($wid){
				pdo_update("j_activity_lucky",array("status"=>$status),array('id'=>$wid));
			}
			message('标识成功！', $this->createWebUrl('lucky', array('id' => $id)), 'success');
		}
		if (checksubmit('delete')) {
			pdo_delete('j_activity_lucky', " from_user  IN  ('".implode("','", $_GPC['select'])."')");
			message('删除成功！', $this->createWebUrl('lucky', array('id' => $id)), 'success');
		}
		if (checksubmit('geprize')) {
			pdo_query("update ".tablename('j_activity_lucky')." set status=2 where from_user  IN  ('".implode("','", $_GPC['select'])."')");
			message('标记成功！', $this->createWebUrl('lucky', array('id' => $id)), 'success');
		}
		if (checksubmit('dealprize')) {
			pdo_query("update ".tablename('j_activity_lucky')." set status=0 where from_user  IN  ('".implode("','", $_GPC['select'])."')");
			message('标记成功！', $this->createWebUrl('lucky', array('id' => $id)), 'success');
		}
		$list = pdo_fetchall("SELECT * FROM ".tablename('j_activity_lucky')." WHERE aid ='".$reply['id']."' order by id desc");
	...
	public function doWebAjax() {
		...
		if($operation=="editmark"){
			$id = intval($_GPC['id']);
			$item = pdo_fetch("SELECT * FROM ".tablename('j_activity_winner')." WHERE id =:id ",array(":id"=>$id));
		...
		if($operation=="savemark"){
			...
			$remark = $_GPC['remark'];
			pdo_update('j_activity_winner',array('reloadmsg'=>$reloadmsg,'remark'=>$remark),array('id'=>$id));
		...
		if($operation=="sendtempmsg"){
			...
			//=====
			$cfg = $this->module['config'];
			if($cfg['user_oauth'])$openid=pdo_fetchcolumn("SELECT openid FROM ".tablename('j_activity_winner')." WHERE from_user = '".$openid."' and aid='".$id."'");
			$url=$_W['siteroot']."app/index.php?i=".$_W['uniacid']."&c=entry&do=view&m=j_activity&id=".$id."&wxref=mp.weixin.qq.com#wechat_redirect";
		...
		if($operation=="getlucky"){
			...
			$reply = pdo_fetch("SELECT * FROM ".tablename('j_activity_reply')." WHERE rid = '$id'");
			$item = pdo_fetchall("SELECT * FROM ".tablename('j_activity_winner')." WHERE aid =:rid $where and from_user not in(SELECT from_user FROM ".tablename('j_activity_lucky')." WHERE aid ='".$reply['id']."') order by id desc",array(":rid"=>$reply['id']));
			$ary=array();
			foreach($item as $row){
				$sex="先生";
			...
			foreach($ary as $row){
				if($i<$num){
					...
					pdo_insert("j_activity_lucky",$data);
					/*$cfg = $this->module['config'];
		...
		if($operation=="getluckylist"){
			...
			$id = intval($_GPC['id']);
			$reply = pdo_fetch("SELECT * FROM ".tablename('j_activity_reply')." WHERE rid = '$id'");
			$item = pdo_fetchall("SELECT * FROM ".tablename('j_activity_winner')." WHERE aid =:rid $where and from_user in(SELECT from_user FROM ".tablename('j_activity_lucky')." WHERE aid ='".$reply['id']."') order by id desc",array(":rid"=>$reply['id']));
			$ary=array();
			foreach($item as $row){
				$sex="先生";
				if($row['gender']==2)$sex="女士";
		...
		if($operation=="sendluckymessage"){
			...
			$reply = pdo_fetch("SELECT * FROM ".tablename('j_activity_reply')." WHERE rid = '$id'");
			$item = pdo_fetchall("SELECT * FROM ".tablename('j_activity_winner')." WHERE aid =:rid and from_user in(SELECT from_user FROM ".tablename('j_activity_lucky')." WHERE aid ='".$reply['id']."') order by id desc",array(":rid"=>$reply['id']));
			$send_ok=0;
			$send_false=0;
			$cfg = $this->module['config'];
		...
		if($operation=="getmsgrecord"){
			$rid = intval($_GPC['rid']);
			$openid = $_GPC['openid'];
			$list=pdo_fetchall("SELECT * FROM ".tablename('j_activity_msgrecord')." WHERE aid =:rid and from_user='".$openid."' order by id asc ",array(":rid"=>$rid));
			$temp=array();
			foreach($list as $row){
				$temp[]=array(
					"content"=>$row["content"],
