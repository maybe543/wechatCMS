...
class Mon_ZLModuleProcessor extends WeModuleProcessor
{
	public function respond()
	{
		...
		$zl = pdo_fetch("select * from " . tablename(DBUtil::$TABLE_ZL) . " where rid=:rid", array(":rid" => $rid));
		if (!empty($zl)) {
			if (TIMESTAMP < $zl['starttime']) {
				return $this->respText("助力活动还未开始哦!");
			}
			$news = array();
			$news [] = array('title' => $zl['new_title'], 'description' => $zl['new_content'], 'picurl' => MonUtil::getpicurl($zl ['new_icon']), 'url' => $this->createMobileUrl('Auth', array('zid' => $zl['id'],'au'=>Value::$REDIRECT_INDEX)));
