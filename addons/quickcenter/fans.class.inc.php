<?php
 class Fans{
    ...
    public function batchGet($weid, $conds, $key, $pindex, $psize, $fields = array('openid as from_user', 'b.nickname', 'avatar', 'credit1', 'follow', 'vip', 'credit2', 'createtime')){
        ...
        $list = pdo_fetchall("SELECT {$fields_str} FROM " . tablename(self :: $t_sys_fans) . " a, " . tablename('mc_members') . " b WHERE acid = :acid AND a.uid = b.uid {$condition} ORDER BY a.uid DESC " . " LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(':acid' => $weid), $key);
        $total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename(self :: $t_sys_fans) . " a, " . tablename('mc_members') . " b WHERE acid = :acid AND a.uid = b.uid {$condition} ", array(':acid' => $weid));
    ...
    public function getActiveUserByTime($weid, $status, $seconds){
        $now = TIMESTAMP;
        $condition = "createtime > {$now} - {$seconds} AND follow={$status} AND acid={$weid}";
