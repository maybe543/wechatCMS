...
class Water_superModuleSite extends WeModuleSite
{
    ...
    public $picstable = 'water_super_pics';
    ...
    public function doMobileWorkOrder()
    {
        ...
        $employee = pdo_fetch("SELECT * FROM " . tablename($this->employeetable) . " WHERE employeestate = 2 and openid =:openid and uniacid = :uniacid", array(
            ':openid' => $workopenid,
            ':uniacid' => $_W['uniacid']
        ));
        if (!$employee) {
        ...
        $systeminfo = pdo_fetch("SELECT shopname,dangmf,weixf,zhifb,iskd FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
    ...
    public function doMobileMyWorkOrder()
    {
        ...
        $systeminfo = pdo_fetch("SELECT shopname,dangmf,weixf,zhifb,isygdj,ddzt1,ddzt2,ddzt3,ddzt4,ddzt5,iskd FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
    ...
    public function doMobileOrderDeal()
    {
        ...
        if ($orderid) {
            $orderinfo = pdo_fetch("SELECT * FROM " . tablename($this->orderstable) . " WHERE id = :id", array(
                ':id' => $orderid
            ));
            if ($orderinfo) {
                $data = array(
                    'orderstate' => $this->orderstate1,
                    'workopenid' => $workopenid
                );
                pdo_update($this->orderstable, $data, array(
                    'id' => $orderid
                ));
                $employee   = pdo_fetch("SELECT * FROM " . tablename($this->employeetable) . " WHERE openid =:openid and uniacid = :uniacid", array(
                    ':openid' => $workopenid,
                    ':uniacid' => $_W['uniacid']
                ));
                $employdata = array(
                    'workstate' => $this->workstate1,
                    'sumorders' => $employee['sumorders'] + 1
                );
                pdo_update($this->employeetable, $employdata, array(
                    'id' => $employee['id']
                ));
    ...
    public function doMobileQueryddh()
    {
        ...
        $systeminfo = pdo_fetch("SELECT shopname,dangmf,weixf,zhifb,isygdj FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
    ...
    public function doMobileQdordercost()
    {
        ...
        if ($orderid) {
            ...
            if ($orderinfo) {
                ...
                $systeminfo = pdo_fetch("SELECT mbxx,utopayordermid,iswww,fuwuname,kefutel FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
                    ':uniacid' => $_W['uniacid']
                ));
                if ($systeminfo['mbxx'] == 2) {
                    if (!empty($systeminfo['utopayordermid'])) {
                        $yhorderurl = $_W['siteroot'] . 'app/' . $this->createMobileUrl('topayorder', array(
                            'id' => $orderid
                        ));
                        $dataxx     = array(
                            'touser' => $orderinfo['openid'],
                            'template_id' => $systeminfo['utopayordermid'],
                            'url' => $yhorderurl,
                            'topcolor' => '#FF0000'
                        );
                        if ($systeminfo['iswww'] == "IT") {
                            $dataxx['data'] = array(
                                ...
                                'remark' => array(
                                    'value' => '请及时付款，如有问题，欢迎拨打客服电话：' . $systeminfo['kefutel'],
                                    'color' => '#173177'
                                )
                            );
                        } else {
    ...
    public function doMobileWorkOrderCancel()
    {
        global $_GPC, $_W;
        $id = intval($_GPC['id']);
        pdo_delete($this->exprestable, array(
            'orderid' => $id
        ));
        pdo_delete($this->orderstable, array(
            'id' => $id
        ));
        $this->doMobileMyWorkOrder();
    }
    public function doMobileOrderDangmf()
    {
        ...
        if ($orderid) {
            ...
            if ($orderinfo) {
                $data   = array(
                    'orderstate' => $this->orderstate2,
                    'paystate' => $this->hasPay,
                    'paytype' => $this->paytype1,
                    'ordercost' => $ordercost
                );
                $member = pdo_fetch("SELECT * FROM " . tablename($this->membertable) . " WHERE openid = :openid and uniacid = :uniacid", array(
                    ':openid' => $orderinfo['openid'],
                    ':uniacid' => $_W['uniacid']
                ));
                if ($member) {
                    pdo_update($this->membertable, array(
                        'real_cost' => $member['real_cost'] + $ordercost,
                        'jifen' => $member['jifen'] + $ordercost
                    ), array(
                        'id' => $member['id']
                    ));
                    pdo_update($this->orderstable, $data, array(
    ...
    public function doMobileIndex()
    {
        ...
        $systeminfo = pdo_fetch("SELECT shopname,kefutel,indexad,xc1,xc2,xc3,goodsinfourl,smsreg,imglb1,imglb2,imglb3,imgurl1,imgurl2,imgurl3,pczjs,pfwfw,pxctp1,pxctp2,pfwfwurl,pczjsurl,pxctp1url,pxctp2url,template FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
        $needreg    = false;
        if ($systeminfo['smsreg'] == 1) {
        ...
        if ($needreg) {
            ...
            $member = pdo_fetch("SELECT * FROM " . tablename($this->membertable) . " WHERE uniacid = :uniacid and openid = :openid ", array(
                ':uniacid' => $_W['uniacid'],
                ':openid' => $openid
            ));
            if (empty($member)) {
    ...
    {
        global $_GPC, $_W;
        $systeminfo = pdo_fetch("SELECT shopname FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
    ...
    {
        global $_GPC, $_W;
        $systeminfo = pdo_fetch("SELECT shopname,goodsinfourl FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
        include $this->template($systeminfo['goodsinfourl']);
    }
    public function doMobileConfirmfee()
    {
        ...
        if ($isajax == 'true') {
            ...
            $systeminfo    = pdo_fetch("SELECT shopname,kefutel,sltxc,sltts,isygdj,confirmfee,imglb1,imglb2 FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
                ':uniacid' => $_W['uniacid']
            ));
            $goodslist     = pdo_fetchall("SELECT * FROM " . tablename($this->goodstable) . " WHERE uniacid = :uniacid and isjj = 1 ", array(
                ':uniacid' => $_W['uniacid']
            ));
            $nojjgoodslist = pdo_fetchall("SELECT * FROM " . tablename($this->goodstable) . " WHERE uniacid = :uniacid and isjj = 0 ", array(
                ':uniacid' => $_W['uniacid']
            ));
            $templatename  = 'confirmfee';
            if ($systeminfo['confirmfee']) {
                $templatename = $systeminfo['confirmfee'];
            }
    ...
    public function doMobileXiadan()
    {
        ...
        if (checksubmit()) {
            ...
            if (is_array($tmparray)) {
                foreach ($tmparray as $item) {
                    ...
                    if (is_array($every)) {
                        ...
                        $orderdetail .= $goods['goodsname'] . "x" . $number . " ";
                    }
                }
            }
            $openid      = $_W['fans']['from_user'];
            $addressid   = intval($_GPC['address_id']);
            $theaddress  = pdo_fetch("SELECT * FROM " . tablename($this->addresstable) . " WHERE id = :id", array(
                ':id' => $addressid
            ));
            ...
            $data        = array(
                ...
                'addressid' => $addressid,
                'customername' => $theaddress['customername'],
                'customertel' => $theaddress['tel'],
                'customercity' => $theaddress['customercity'],
                'customerarea' => $theaddress['customerarea'],
                'xiangxdz' => $theaddress['xiangxdz'],
                'shouhrdz' => $theaddress['shouhrdz'],
                'ordertype' => $this->ordertype01,
                'detail' => $orderdetail,
                'ordercost' => floatval($ordercost),
                'paystate' => $this->unPay,
                'orderstate' => $this->orderstate0
            );
            ...
            $systeminfo = pdo_fetch("SELECT smsdx,mbxx,smsuid,smspwd,unewordermid,wnewordermid,kefutel,iswww FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
                ':uniacid' => $_W['uniacid']
            ));
            if ($systeminfo['mbxx'] == 2) {
                if (!empty($systeminfo['unewordermid'])) {
                    $yhorderurl = $_W['siteroot'] . 'app/' . $this->createMobileUrl('ordering', array(
                        'openid' => $openid
                    ));
                    $data       = array(
                        'touser' => $openid,
                        'template_id' => $systeminfo['unewordermid'],
                        'url' => $yhorderurl,
                        'topcolor' => '#FF0000'
                    );
                    if ($systeminfo['iswww'] == "IT") {
                        $data['data'] = array(
                            ...
                            'remark' => array(
                                'value' => '谢谢您的惠顾，如有问题，欢迎拨打客服电话：' . $systeminfo['kefutel'],
                                'color' => '#173177'
                            )
                        );
                    } else {
                ...
                if (!empty($systeminfo['wnewordermid'])) {
                    $data2 = array(
                        'touser' => '',
                        'template_id' => $systeminfo['wnewordermid'],
                        'url' => '',
                        'topcolor' => '#FF0000'
                    );
                    if ($systeminfo['iswww'] == "IT") {
                        $data2['data'] = array(
                            ...
                            'keyword5' => array(
                                'value' => $theaddress['customername'],
                                'color' => '#173177'
                            ),
                            'remark' => array(
                                'value' => $theaddress['customercity'] . '-' . $theaddress['customerarea'] . '-' . $theaddress['xiangxdz'],
                                'color' => '#173177'
                            )
                        );
                    } else {
                    ...
                    foreach ($workers as $work) {
                        ...
                        if ($send) {
                            ...
                            $data2['url']    = $_W['siteroot'] . 'app/' . $this->createMobileUrl('workorder', array(
                                'workopenid' => $work['openid']
                            ));
                            $token = $this->getToken();
            ...
            $openid       = $_W['fans']['from_user'];
            $addressList  = pdo_fetchall("SELECT * FROM " . tablename($this->addresstable) . " WHERE openid = :openid", array(
                ':openid' => $openid
            ));
            $firstaddress = pdo_fetch("SELECT * FROM " . tablename($this->addresstable) . " WHERE openid = :openid", array(
                ':openid' => $openid
            ));
    ...
    public function doMobileNewaddress()
    {
        ...
        if (checksubmit()) {
            ...
            $data = array(
                'customername' => $_GPC['customername'],
                'tel' => $_GPC['tel'],
                'customercity' => $_GPC['customercity'],
                'customerarea' => $_GPC['customerarea'],
                'xiangxdz' => $_GPC['xiangxdz'],
                'shouhrdz' => $_GPC['shouhrdz']
            );
            if (empty($addressid)) {
                ...
                pdo_update($this->addresstable, $data, array(
                    'id' => $addressid
                ));
            }
            if ($_GPC['from'] == 'U') {
            ...
            $systeminfo = pdo_fetch("SELECT shopname,cityunicode,areaunicode,iskd FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
                ':uniacid' => $_W['uniacid']
            ));
    ...
    public function doMobileDeleteAddressAjax()
    {
        ...
        pdo_delete($this->addresstable, array(
            'id' => $addressid
        ));
        die(json_encode(array(
            'code' => 2
    ...
    public function doMobileOrdering()
    {
        ...
        $systeminfo = pdo_fetch("SELECT shopname,dangmf,weixf,zhifb,fuwuname,ddzt0,ddzt1,ddzt2,ddzt3,ddzt4,isygdj FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
    ...
    public function doMobileOrderend()
    {
        ...
        $systeminfo = pdo_fetch("SELECT shopname,fuwuname,ddzt5 FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
        $orderList  = pdo_fetchall("SELECT * FROM " . tablename($this->orderstable) . " WHERE ordertype = 'wash' and orderstate = 5 and openid = :openid order by id desc", array(
            ':openid' => $openid
        ));
        $numid      = $_W['uniacid'];
        if (empty($numid)) {
    ...
    {
        global $_GPC, $_W;
        $numid = intval($_GPC['numid']);
        if (empty($_W['uniacid'])) {
            $_W['uniacid'] = $numid;
        }
        $systeminfo     = pdo_fetch("SELECT shopname,fuwuname,ddzt0,ddzt1,ddzt2,ddzt3,ddzt4,ddzt5,iskd FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
        $orderid        = intval($_GPC['id']);
        $orderinfo      = pdo_fetch("SELECT * FROM " . tablename($this->orderstable) . " WHERE id = :id", array(
        ...
        $employee       = pdo_fetch("SELECT employeename,tel FROM " . tablename($this->employeetable) . " WHERE employeestate = 2 and openid =:openid and uniacid = :uniacid", array(
            ':openid' => $orderinfo['workopenid'],
            ':uniacid' => $_W['uniacid']
        ));
        $expresinfoList = pdo_fetchall("SELECT * FROM " . tablename($this->exprestable) . " WHERE orderid = :orderid order by expresstime desc ", array(
            ':orderid' => $orderid
        ));
        $beforelist     = pdo_fetchall("SELECT * FROM " . tablename($this->picstable) . " WHERE orderid = :orderid and state = 2  order by id desc limit 3 ", array(
            ':orderid' => $orderid
        ));
        $afterlist      = pdo_fetchall("SELECT * FROM " . tablename($this->picstable) . " WHERE orderid = :orderid  and state = 5 order by id desc limit 3 ", array(
            ':orderid' => $orderid
        ));
    ...
    public function doMobileToPayOrder()
    {
        ...
        $systeminfo = pdo_fetch("SELECT * FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
    ...
    public function payResult($params)
    {
        ...
        if ($ordertype != 'recharge') {
            ...
            $systeminfo = pdo_fetch("SELECT mbxx,upayordermid,iswww FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
                ':uniacid' => $_W['uniacid']
            ));
            if ($systeminfo['mbxx'] == 2 && !empty($systeminfo['upayordermid'])) {
                ...
                $dataxx     = array(
                    'touser' => $openid,
                    'template_id' => $systeminfo['upayordermid'],
                    'url' => $yhorderurl,
                    'topcolor' => '#FF0000'
                );
                if ($systeminfo['iswww'] == "IT") {
            ...
            load()->model('mc');
            $status = pdo_fetchcolumn("SELECT status FROM " . tablename('mc_credits_recharge') . " WHERE tid = :tid", array(
                ':tid' => $params['tid']
            ));
            if (empty($status)) {
                $fee  = $params['fee'];
                $data = array(
                    'status' => $params['result'] == 'success' ? 1 : -1
                );
                if ($params['type'] == 'wechat') {
                    $data['transid'] = $params['tag']['transaction_id'];
                    $params['user']  = mc_openid2uid($params['user']);
                }
                pdo_update('mc_credits_recharge', $data, array(
                    'tid' => $params['tid']
                ));
                if ($params['result'] == 'success' && $params['from'] == 'return') {
                    ...
                    if (empty($credit)) {
                        ...
                        $systeminfo   = pdo_fetch("SELECT recharge,shopname FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
                            ':uniacid' => $_W['uniacid']
                        ));
    ...
    {
        global $_GPC, $_W;
        $systeminfo = pdo_fetch("SELECT shopname,cardlogo,kefutel,kefuwx,yjfkurl FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
        $openid     = $_W['fans']['from_user'];
        if (empty($openid)) {
        ...
        $userinfo = pdo_fetch("SELECT cardnumber FROM " . tablename($this->membertable) . " WHERE uniacid = :uniacid and openid = :openid", array(
            ':uniacid' => $_W['uniacid'],
            ':openid' => $openid
        ));
    ...
    public function doMobileUaddress()
    {
        ...
        $addressList = pdo_fetchall("SELECT * FROM " . tablename($this->addresstable) . " WHERE uniacid = :uniacid and openid = :openid", array(
            ':uniacid' => $_W['uniacid'],
            ':openid' => $openid
        ));
    ...
    {
        global $_GPC, $_W;
        $systeminfo = pdo_fetch("SELECT shopname,kefutel,kefuwx FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
    ...
    public function doWebShowEmployee()
    {
        ...
        $citygory = pdo_fetchall($sql1, array(
            ':uniacid' => $_W['uniacid']
        ), 'id');
        if (!empty($citygory)) {
            $parent = array();
        ...
        $areagory = pdo_fetchall($sql2, array(
            ':uniacid' => $_W['uniacid']
        ), 'id');
        if (!empty($areagory)) {
            $children = array();
        ...
        if ($_GPC['op'] == 'delete') {
            ...
            pdo_delete($this->employeetable, array(
                'id' => $employeeid
            ));
            message('删除成功！', referer(), 'success');
        }
        if ($_GPC['op'] == 'update') {
            ...
            pdo_update($this->employeetable, array(
                'employeestate' => 2
            ), array(
                'id' => $employeeid
            ));
        ...
        if (checksubmit()) {
            ...
            $data = array(
                'employeename' => $_GPC['employeename'],
                'tel' => $_GPC['tel'],
                'sumorders' => $_GPC['sumorders'],
                'cityid' => $_GPC['fuzqy']['parentid'],
                'areaid' => $_GPC['fuzqy']['childid'],
                'city' => $cityname,
                'area' => $areaname
            );
            pdo_update($this->employeetable, $data, array(
                'id' => $employeeid
            ));
    ...
    public function doWebShowMember()
    {
        ...
        if (checksubmit()) {
            $data = array(
                ...
                'tel' => $_GPC['tel'],
                'real_cost' => floatval($_GPC['real_cost']),
                'balance' => floatval($_GPC['balance'])
            );
            pdo_update($this->membertable, $data, array(
                'id' => $memberid
            ));
            message('更新成功！', referer(), 'success');
        }
    ...
    public function doWebAddGoods()
    {
        ...
        if ($_GPC['op'] == 'delete') {
            ...
            pdo_delete($this->goodstable, array(
                'id' => $goodsid
            ));
            message('删除成功！', referer(), 'success');
        }
        if (checksubmit()) {
            $data = array(
                ...
                'goodsinfo' => htmlspecialchars_decode($_GPC['goodsinfo']),
                'goodsphoto' => $_GPC['goodsphoto'],
                'danwei' => $_GPC['danwei'],
                'biaoqian' => $_GPC['biaoqian'],
                'scprice' => floatval($_GPC['scprice']),
                'isjj' => intval($_GPC['isjj']),
                'goodsprice' => floatval($_GPC['goodsprice'])
            );
            if (!empty($goodsid)) {
                pdo_update($this->goodstable, $data, array(
                    'id' => $goodsid
                ));
            } else {
                $data['uniacid'] = $_W['uniacid'];
    ...
    public function doWebAddCity()
    {
        ...
        if ($_GPC['op'] == 'delete') {
            ...
            pdo_delete($this->citytable, array(
                'id' => $cityid
            ));
            pdo_delete($this->areatable, array(
                'cityid' => $cityid
            ));
        ...
        if (checksubmit()) {
            $data      = array(
                ...
                'cityinfo' => htmlspecialchars_decode($_GPC['cityinfo']),
                'cityphoto' => $_GPC['cityphoto'],
                'cityunicode' => json_encode($_GPC['cityname'])
            );
            $isNewcity = false;
            if (!empty($cityid)) {
                pdo_update($this->citytable, $data, array(
                    'id' => $cityid
                ));
            } else {
                $data['uniacid'] = $_W['uniacid'];
    ...
    public function doWebAddArea()
    {
        ...
        if (checksubmit()) {
            $data = array(
                'areaname' => $_GPC['areaname'],
                'areaunicode' => json_encode($_GPC['areaname'])
            );
            if (!empty($areaid)) {
                pdo_update($this->areatable, $data, array(
                    'id' => $areaid
                ));
            } else {
                $data['uniacid'] = $_W['uniacid'];
    ...
    public function dowebAddMessage()
    {
        ...
        if (checksubmit()) {
            $data = array(
                'unewordermid' => $_GPC['unewordermid'],
                'upayordermid' => $_GPC['upayordermid'],
                'utopayordermid' => $_GPC['utopayordermid'],
                'wnewordermid' => $_GPC['wnewordermid'],
                'smsuid' => $_GPC['smsuid'],
                'smspwd' => $_GPC['smspwd'],
                'smsyzmb' => $_GPC['smsyzmb'],
                'iswww' => $_GPC['iswww'],
                'printcode' => $_GPC['printcode'],
                'printak' => $_GPC['printak'],
                'printsk' => $_GPC['printsk'],
                'printid' => $_GPC['printid'],
                'qnscode' => $_GPC['qnscode'],
                'qnym' => $_GPC['qnym'],
                'qnak' => $_GPC['qnak'],
                'qnsk' => $_GPC['qnsk']
            );
    ...
    public function dowebSystemOptions()
    {
        global $_W, $_GPC;
        load()->func('tpl');
        $shop = pdo_fetch("SELECT id,needaudit,isygdj,dangmf,weixf,zhifb,smsdx,smsreg,mbxx,iskd FROM " . tablename($this->shoptable) . " WHERE uniacid= " . $_W['uniacid']);
        if (empty($shop)) {
            message('请先点击商家设置完善店铺信息！');
        }
        if (checksubmit()) {
            $data = array(
                'needaudit' => $_GPC['needaudit'],
                'isygdj' => intval($_GPC['isygdj']),
                'dangmf' => $_GPC['paytype'][0],
                'weixf' => $_GPC['paytype'][1],
                'zhifb' => $_GPC['paytype'][2],
                'smsdx' => $_GPC['tztype'][0],
                'mbxx' => $_GPC['tztype'][1],
                'smsreg' => $_GPC['smsreg'],
                'iskd' => $_GPC['iskd']
    ...
    public function dowebFengge()
    {
        ...
        if (checksubmit()) {
            $data = array(
                ...
                'imglb2' => $_GPC['imglb2'],
                'imglb3' => $_GPC['imglb3'],
                'imgurl1' => $_GPC['imgurl1'],
                'imgurl2' => $_GPC['imgurl2'],
                'imgurl3' => $_GPC['imgurl3'],
                'template' => $_GPC['template'],
                'confirmfee' => $_GPC['confirmfee'],
                'goodsinfourl' => $_GPC['goodsinfourl'],
                'pfwfw' => $_GPC['pfwfw'],
                'pczjs' => $_GPC['pczjs'],
                'pxctp1' => $_GPC['pxctp1'],
                'pxctp2' => $_GPC['pxctp2'],
                'pfwfwurl' => $_GPC['pfwfwurl'],
                'pczjsurl' => $_GPC['pczjsurl'],
                'pxctp1url' => $_GPC['pxctp1url'],
                'pxctp2url' => $_GPC['pxctp2url']
            );
            pdo_update($this->shoptable, $data, array(
                'id' => $shop['id']
            ));
    ...
    public function dowebAddShop()
    {
        ...
        if (checksubmit()) {
            $data = array(
                ...
                'fuwuname' => $_GPC['fuwuname'],
                'cardlogo' => $_GPC['cardlogo'],
                'addemployeepwd' => $_GPC['addemployeepwd'],
                'kefuwx' => $_GPC['kefuwx'],
                'recharge' => $_GPC['recharge'],
                'yjfkurl' => $_GPC['yjfkurl']
            );
            if (!empty($shopid)) {
                pdo_update($this->shoptable, $data, array(
                    'id' => $shopid
    ...
    {
        global $_W;
        $orderExpress = pdo_fetch("SELECT * FROM " . tablename($this->exprestable) . " WHERE uniacid = :uniacid and  orderid= :orderid and orderstate =:orderstate", array(
            ':uniacid' => $_W['uniacid'],
            ':orderid' => $orderid,
    ...
    private function createNum()
    {
        global $_W;
        $f       = date('Ymd');
        $rnumber = pdo_fetch("SELECT * FROM " . tablename($this->rnumbertbale) . " WHERE uniacid = :uniacid order by id desc ", array(
            ':uniacid' => $_W['uniacid']
        ));
        $maxNum  = 0;
        if (!empty($rnumber)) {
        ...
        pdo_insert($this->rnumbertbale, array(
            'uniacid' => $_W['uniacid']
        ));
        $lastnum = $maxNum;
        if ($rnumber['id'] < 9) {
    ...
    private function createCardNum()
    {
        global $_W;
        $f       = '188';
        $rnumber = pdo_fetch("SELECT * FROM " . tablename($this->cardnumbertbale) . " WHERE uniacid = :uniacid order by id desc ", array(
            ':uniacid' => $_W['uniacid']
        ));
        $maxNum  = 0;
        if (!empty($rnumber)) {
        ...
        pdo_insert($this->cardnumbertbale, array(
            'uniacid' => $_W['uniacid']
        ));
        $lastnum = $maxNum;
        if ($rnumber['id'] < 9) {
    ...
    public function getOrderWorkerName($openid)
    {
        global $_W;
        $result = pdo_fetch("SELECT employeename FROM " . tablename($this->employeetable) . " WHERE openid = '{$openid}' ");
    ...
    public function doMobileSendVerifyCode()
    {
        ...
        $systeminfo       = pdo_fetch("SELECT smsuid,smspwd,smsyzmb FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
        if (empty($systeminfo['smsuid']) || empty($systeminfo['smspwd'])) {
            die(json_encode(array(
                "result" => 0,
                "code" => 'sms帐号密码为空'
            )));
    ...
    public function doMobileRecharge()
    {
        ...
        $systeminfo            = pdo_fetch("SELECT recharge,shopname,imglb1 FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $_W['uniacid']
        ));
        $tmparray              = explode("#", $systeminfo['recharge']);
        $charge1               = 0;
        ...
        if (checksubmit()) {
            ...
            if (empty($chargerecord)) {
                ...
                if (!pdo_insert('mc_credits_recharge', $chargerecord)) {
                    message('创建充值订单失败，请重试！', url('entry', array(
                        'm' => 'recharge',
                        'do' => 'pay'
                    )), 'error');
                }
    ...
    public function doWebPrintOrder()
    {
        ...
        $pconfig = pdo_fetch("SELECT printcode,printak,printsk,printid FROM " . tablename($this->shoptable) . " WHERE uniacid= " . $_W['uniacid']);
        if (!empty($pconfig['printsk'])) {
            $msg               = "订单编号：" . $order['ordercode'] . "\r\n接单人：" . $employeename . "\r\n下单时间：" . $order['ordertime'] . "\r\n服务时间：" . $order['fuwuriqi'] . $order['fuwushijian'] . "\r\n客户名称：" . $order['customername'] . "\r\n客户电话：" . $order['customertel'] . "\r\n客户地址：" . $order['customercity'] . $order['customerarea'] . $order['xiangxdz'] . "\r\n订单明细：" . $order['detail'] . "\r\n订单金额：" . $order['ordercost'] . "元\r\n";
            $apiKey            = $pconfig['printak'];
            $mKey              = $pconfig['printsk'];
            $partner           = $pconfig['printid'];
            $machine_code      = $pconfig['printcode'];
            $ti                = time();
            $params            = array(
                'partner' => $partner,
            ...
            $data = rtrim($p, '&');
            $rmsg = $this->liansuo_post($url, $data);
            die($rmsg);
        } else {
            $result = $this->printOrder($order, $employeename, $pconfig);
            if ($result == 0) {
                die(json_encode(array(
                    "result" => 0,
                    "state" => "打印成功"
    ...
    public function doMobilePrintOrder()
    {
        global $_GPC, $_W;
        $orderid      = intval($_GPC['orderid']);
        $order        = pdo_fetch("SELECT * FROM " . tablename($this->orderstable) . " WHERE id = " . $orderid);
        ...
        $feiyin = pdo_fetch("SELECT MEMBER_CODE,FEYIN_KEY,DEVICE_NO FROM " . tablename($this->shoptable) . " WHERE uniacid= " . $_W['uniacid']);
        $result = $this->printOrder($order, $employeename, $feiyin);
        if ($result == 0) {
            die(json_encode(array(
                "result" => 0,
                "msg" => "打印成功"
    ...
    public function printOrder($order, $name, $feiyin)
    {
        $msgNo   = time() + 1;
        $cost    = intval($order['ordercost'] * 100);
        $msgInfo = array(
            'memberCode' => $feiyin['printid'],
            'extra5 ' => $order['ordertime'],
            'extra1' => $order['ordercode'],
            'extra2' => $order['fuwuriqi'],
            'extra3' => $order['fuwushijian'],
            'extra4' => $name,
            'customerName' => $order['customername'],
            'customerPhone' => $order['customertel'],
            'customerAddress' => $order['customercity'] . $order['customerarea'] . $order['xiangxdz'],
            'customerMemo' => $order['detail'],
            'msgDetail' => '订单@' . $cost . '@1',
            'deviceNo' => $feiyin['printcode'],
            'msgNo' => $msgNo
        );
        return $this->sendFormatedMessage($msgInfo, $feiyin);
    }
    function sendFormatedMessage($msgInfo, $feiyin)
    {
        $msgInfo['reqTime']      = number_format(1000 * time(), 0, '', '');
        $content                 = $msgInfo['memberCode'] . $msgInfo['customerName'] . $msgInfo['customerPhone'] . $msgInfo['customerAddress'] . $msgInfo['customerMemo'] . $msgInfo['msgDetail'] . $msgInfo['deviceNo'] . $msgInfo['msgNo'] . $msgInfo['reqTime'] . $feiyin['printak'];
    ...
    public function doMobileToupload()
    {
        global $_GPC, $_W;
        load()->app('tpl');
        if ($_W['uniacid'] == 0) {
            message('uniacid is 0');
        }
        ...
        include $this->template('wuploadpic');
    }
    public static function imgUrlHandle($uri, $type, $ym)
    {
        global $_W;
        switch ($type) {
            case 'local':
                $url = $_W['siteroot'] . 'addons/water_super/template/upload/images/' . $uri;
                break;
            case 'qiniu':
                $url = "http://" . $ym . "/" . $uri;
                break;
    ...
    {
        $settings = pdo_fetch("select * from " . tablename($this->shoptable));
        if ($settings === false) {
            return false;
        }
        return $settings;
    ...
    public function doMobilePicupload()
    {
        ...
        if (empty($orderid)) {
            ...
            $this->echoJson(array(
                "res" => '101',
                'msg' => 'no orderid param'
            ));
        }
        $orderinfo      = pdo_fetch("SELECT uniacid FROM " . tablename($this->orderstable) . " WHERE id = :id", array(
            ':id' => $orderid
        ));
        $systeminfo     = pdo_fetch("SELECT qnscode,qnym,qnak,qnsk FROM " . tablename($this->shoptable) . " WHERE uniacid = :uniacid", array(
            ':uniacid' => $orderinfo['uniacid']
        ));
        $set['qnscode'] = $systeminfo['qnscode'];
        $set['qnym']    = $systeminfo['qnym'];
        $set['qnak']    = $systeminfo['qnak'];
        $set['qnsk']    = $systeminfo['qnsk'];
        if (empty($set['qnscode']) || empty($set['qnym']) || empty($set['qnak']) || empty($set['qnsk'])) {
            header("HTTP/1.1 500 Internal Server Error");
            $this->echoJson(array(
                "res" => '101',
                'msg' => '7 niu seting'
        ...
        $filename = 'we7_water_super/' . $filename;
        ...
        if ($err) {
            ...
            $data['uniacid'] = $_W['uniacid'];
            $data['imgurl']  = $this->imgUrlHandle($filename, $upload_way, $set['qnym']);
            $res             = pdo_insert($this->picstable, $data);
            if ($res) {
                $this->echoJson(array(
                    "res" => '100',
                    'msg' => 'success'
