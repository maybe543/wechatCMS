...
function replace_em($str){
	$str = preg_replace('/\[em_(.*?)\]/', '<img src="../addons/meepo_bbs/template/mobile/default/lib/qqface/arclist/$1.gif" style="width:24px !important;height:24px; !important">', $str);
	return $str;
}
...
function getAdv($typeid = 0){
	global $_W;
	if(empty($typeid)){
		$sql = "SELECT * FROM ".tablename('meepo_bbs_adv')." WHERE uniacid = :uniacid AND enabled = :enabled AND typeid = :typeid ORDER BY RAND() LIMIT 1";
		$params = array(':uniacid'=>$_W['uniacid'],':enabled'=>1,':typeid'=>0);
		$advs = pdo_fetch($sql,$params);
	}else{
		$sql = "SELECT * FROM ".tablename('meepo_bbs_adv')." WHERE uniacid = :uniacid AND typeid = :typeid AND enabled = :enabled ORDER BY RAND() LIMIT 1";
		$params = array(':uniacid'=>$_W['uniacid'],':typeid'=>$typeid,':enabled'=>1);
		$advs = pdo_fetch($sql,$params);
	}
