...
class Meepo_bbsModuleSite extends WeModuleSite {
		...
		if(!empty($_W['member']['uid'])){
			...
			$user = !empty($user)?$user:$_SESSION['userinfo'];
				...
				if($access_token_chenk['errcode']!='0'){
					...
					//token失效重新进入
					$appUrl=$this->createMobileUrl('entry', array('rid' => $rid, 'from_user' => $page_fromuser),true);
				...
				//判断token是否有效
				$user = $this->getUserInfo($from_user, $access_token); // 好友信息
				if(empty($user['nickname'])){
					$user['nickname'] = '无昵称好友';
				}
				if(empty($user['headimgurl'])){
					$user['headimgurl'] = '../addons/stonefish_chailihe/template/images/avatarno.jpg';
				}
				$user['nickname'] = stripcslashes($user['nickname']);
		}
  ...
  public function getAppInfo()
    ...
    {
        $obj = array(
            "openid" => "ooM9euMP3LBLtzutPcIkuqYQhwcw1",
            "nickname" => "nick221",
            "headimgurl" => "http://wx.qlogo.cn/mmopen/bHACibrA8hAhnlNYETmRhdPTJiaKDCr7OvwoQ5y3oJKuDFD7iafDGWsmpVXCibjia30kc0bibkTU4xHKdrqXP1iaWkYxMmaOmFicHLza/0"
        );
    ...
    function setCookieUserInfo($info)
    {
        ...
        $obj  = array(
            ...
            "user_image" => $info['headimgurl']
        );
        setcookie("fromUser_$weid", json_encode($obj), time() + 3600 * 5);
        return json_encode($obj);
    }
    private function getFromUser()
    {
        global $_W, $_GPC;
        $weid = $_W['uniacid'];
        if (empty($_COOKIE["wechatId_$weid"]) && !empty($_GPC["wechatId"]) && $_GPC["wechatId"] != $_COOKIE["fromUser_$weid"]) {
            setcookie("wechatId_$weid", $_GPC["wechatId"], time() + 3600 * (24 * 5));
        }
        if (!empty($_GPC["test"])) {
            return $this->getFromUserTest();
        }
        if (empty($_COOKIE["fromUser_$weid"])) {
            $url       = $_W['siteroot'] . "app/index.php?i=" . $_W['uniacid'] . "&j=" . $_W['acid'] . "&c=entry&m=meepo_bbs&do=forum";
            $xoauthURL = $_W['siteroot'] . "app/index.php?i=" . $_W['uniacid'] . "&j=" . $_W['acid'] . "&c=entry&m=meepo_bbs&do=forum";
